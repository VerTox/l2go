...
import (
	...
	"github.com/VerTox/l2go/config"
	clientpackets2 "github.com/VerTox/l2go/internal/gameserver/clientpackets"
	"github.com/VerTox/l2go/internal/gameserver/models"
	serverpackets2 "github.com/VerTox/l2go/internal/gameserver/serverpackets"
...
func (g *GameServer) kickClient(client *models.Client) {
	...
	fmt.Println("The inclient has been successfully kicked from the server.")
}

func (g *GameServer) handleClientPackets(client *models.Client) {
	fmt.Println("A inclient is trying to connect...")
	defer g.kickClient(client)

	// Client protocol version
	_, data, err := client.Receive(false)
	protocolVersion := clientpackets2.NewProtocolVersion(data)
	...
	fmt.Println("Sending the Xor Key to the inclient...")

	buffer := serverpackets2.NewCryptInitPacket()
	...
	for {
		...
		case 0x08:
			...
			buffer := serverpackets2.NewCharListPacket()
		...
		case 0x0e:
			...
			buffer := serverpackets2.NewCharTemplatePacket()
			err := client.Send(buffer)
		...
		case 0x0b:
			character := clientpackets2.NewCharacterCreate(data)

			fmt.Printf("Created a new character : %s\n", character.Name)

			// ACK
			buffer := serverpackets2.NewCharCreateOkPacket()
			err := client.Send(buffer)
